...
def convert_onnx2tensorrt(onnx_file_path, engine_file_path, d_type):
    if d_type == np.float16:
        command = 'trtexec --onnx=' + onnx_file_path + ' --saveEngine=' + \
                  engine_file_path + ' --workspace=2048 --explicitBatch --fp16'
    else:
        command = 'trtexec --onnx=' + onnx_file_path + ' --saveEngine=' + \
...
def sigmoid(x):
  return np.round(1 / (1 + math.exp(-x)))
...
def main(args):
    ...
    input_batch = np.zeros((1, 3, 300, 300), d_type)
    output = np.empty([1], dtype=d_type)
    ...
    if not os.path.isfile(trt_path) or regen:
        convert_onnx2tensorrt(onnx_file_path = onnx_path, engine_file_path = trt_path, d_type = d_type)
    ...
    logger.info('Validation result: ==== Time): %.5f' % np.mean(time_set[5:]))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--onnx_path', default="efficientnet16_onnx_model.onnx", help='onnx model path')
    parser.add_argument('--trt_path', default="efficientnet16_engine.trt", help='tensorrt engine path')

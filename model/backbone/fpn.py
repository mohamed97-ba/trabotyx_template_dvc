...
from model.backbone.aspp import ASPP, ASPP_Bottleneck
...
class ResNet(nn.Module):
    ...
    def __init__(self, arch, block, layers, output_stride, BatchNorm, num_classes, pretrained=True):
        self.inplanes = 64
        self.layers = layers
        self.arch = arch
        ...
        if self.arch == 'resnet18':
            self.toplayer = nn.Conv2d(512, num_classes, kernel_size=1, stride=1, padding=0)
            self.latlayer1 = nn.Conv2d(256, num_classes, kernel_size=1, stride=1, padding=0)
            self.latlayer2 = nn.Conv2d( 128, num_classes, kernel_size=1, stride=1, padding=0)
            self.latlayer3 = nn.Conv2d( 64, num_classes, kernel_size=1, stride=1, padding=0)
        else: 
            self.toplayer = nn.Conv2d(256, 256, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, input):
        ...
        x = self.layer4(c4) #x16
        x = self.aspp(x) # ASPP Bottleneck to go deeper in the network

        # Top-down
        x = self.toplayer(x)
        x = nn.functional.interpolate(x, size=c4.size()[2:], mode='bilinear',  align_corners=True) + self.latlayer1(c4)
        x = nn.functional.interpolate(x, size=c3.size()[2:], mode='bilinear',  align_corners=True) + self.latlayer2(c3)
        x = nn.functional.interpolate(x, size=c2.size()[2:], mode='bilinear',  align_corners=True) + self.latlayer3(c2)
...
def FPN50(num_classes, output_stride, BatchNorm=nn.BatchNorm2d, pretrained=True):
